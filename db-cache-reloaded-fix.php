	...
	class DBCacheReloaded {
		...
		public function __construct() {
			...
			if ( is_admin() ) {
				...
				// Catch options page
				add_action( 'load-settings_page_' . substr( plugin_basename( __FILE__ ), 0, - 4 ), array(
					$this,
					'load_settings_page'
				) );
				...
				add_action( 'ozh_adminmenu_icon_' . plugin_basename( __FILE__ ), array(
					$this,
					'ozh_adminmenu_icon'
				) );
			}
		...
		function options_page() {
            ...
            <div class="wrap">
                <a href="http://www.ivankristianto.com/">
                    <div id="db-cache-reloaded-icon"
                         style="background: url(<?php echo plugin_dir_url( __FILE__ ) ?>dbcache-icon.png) no-repeat;"
                         class="icon32"><br/></div>
                </a>
                <form method="post">
					<?php wp_nonce_field( 'db-cache-reloaded-update-options' ); ?>
                    <h2><?php _e( 'DB Cache Reloaded Fix - Options', 'db-cache-reloaded' ); ?></h2>
                    ...
                    <p class="submit">
                        <input class="button" type="submit" name="save"
                               value="<?php _e( 'Save', 'db-cache-reloaded' ); ?>">
                        <input class="button" type="submit" name="clear"
                               value="<?php _e( 'Clear the cache', 'db-cache-reloaded' ); ?>">
                        <input class="button" type="submit" name="clearold"
                               value="<?php _e( 'Clear the expired cache', 'db-cache-reloaded' ); ?>">
                    </p>
                </form>
            </div>
			<?php
